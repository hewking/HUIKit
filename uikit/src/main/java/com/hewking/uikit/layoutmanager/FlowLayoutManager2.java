package com.hewking.uikit.layoutmanager;
...
public class FlowLayoutManager2 extends RecyclerView.LayoutManager {
    ...
    public static final String TAG = "FlowLayoutManager2";
...
/*    private class FlowItem {
    ...
    public int scrollVerticallyBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (mVerticalOffset + realOffset < 0) {//上边界
            ...
            //利用最后一个子View比较修正
            View lastChild = getChildAt(getChildCount() - 1);
            String s = String.format("last pos %d itemcount : %d", getPosition(lastChild), getItemCount() -1);
            L.d(s,TAG);
            if (getPosition(lastChild) == getItemCount() - 1) {
                int gap = getHeight() - getPaddingBottom() - getDecoratedBottom(lastChild);
                if (gap > 0) {
