...
public class RBaseItemDecoration extends RecyclerView.ItemDecoration {
    ...
    boolean drawFirstVLine = false; // 第一个位置是普通分割线 10 倍并在头部
    ...
    public RBaseItemDecoration setDrawFirstSpace(boolean isDraw) {
        this.drawFirstVLine = isDraw;
        return this;
    }
    ...
    private void darwFirstSpace(Canvas c, View view) {
        final RecyclerView.LayoutParams p = (RecyclerView.LayoutParams) view.getLayoutParams();
        Drawable firstSpaceDrawable = new ColorDrawable(dividerColor);
        firstSpaceDrawable.setBounds(view.getLeft(),
                (int)(view.getTop() - p.topMargin - 20 * mDividerSize),view.getRight(),
                view.getTop());
        drawDrawable(c,firstSpaceDrawable);
    }
...
//                    }
                    ...
                    // 第一个位置是否需要 默认长分割
                    if (drawFirstVLine) {
                        outRect.set(0,(int)mDividerSize * 20,0,(int)mDividerSize);
                    } else {
