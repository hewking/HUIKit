 ...
 * 自定义seekbar可当做时间轴
 */
public class SectionStepView extends View {
    ...
    private float mBarHeight;// 节点的半径
    ...
    public SectionStepView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    public SectionStepView(Context context) {
        super(context);
    }

    public SectionStepView(Context context, AttributeSet attrs) {
        super(context, attrs);

        TypedArray typedArray = context.obtainStyledAttributes(attrs,
                R.styleable.sectionStep);
        mBarColor = typedArray.getColor(R.styleable.sectionStep_barColor,
                Color.BLUE);
        mStepSelectedColor = typedArray.getColor(
                R.styleable.sectionStep_stepSelectedColor, Color.RED);
        mStepNormalColor = typedArray.getColor(
                R.styleable.sectionStep_stepNormalColor, Color.BLUE);
        mProgressColor = typedArray.getColor(R.styleable.sectionStep_progressColor,
                Color.RED);
        mStepCount = typedArray.getInt(R.styleable.sectionStep_stepCount, 3);
        mBarHeight = typedArray.getDimension(R.styleable.sectionStep_barHeight,
                TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 15,
                        context.getResources().getDisplayMetrics()));
        mCurrentStep = typedArray.getInt(R.styleable.sectionStep_currentStep, 0);
        mCanDrag = typedArray.getBoolean(R.styleable.sectionStep_canDrag, true);
        mStepRadius = typedArray.getDimension(R.styleable.sectionStep_stepRadius,
                TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 15,
                        context.getResources().getDisplayMetrics()));
        mProgress = (float) mCurrentStep / (mStepCount - 1);
        typedArray.recycle();
    ...
    private void drawSteps(Canvas canvas) {
        ...
        for (int i = 0; i < mStepCount; i++) {
            ...
            // 下面开始画圆
            tmpRadius = mStepRadius;
            if ((float) i / (mStepCount - 1) <= mProgress) {
                mPaint.setColor(mStepSelectedColor);
            } else {
                mPaint.setColor(mStepNormalColor);
            }
            RectF oval = new RectF(pivotX - tmpRadius, getPaddingTop(),
                    pivotX + tmpRadius, getPaddingTop() + tmpRadius * 2);
/*			// 选中当前 圆点 半径二倍
            if (mCurrentStep == i ) {
				tmpRadius = mStepRadius * 2;
				oval.set(pivotX - tmpRadius, getPaddingTop() - mStepRadius,
						pivotX + tmpRadius, getPaddingTop() + mStepRadius * 3);
			}*/
    ...
    private void drawProgress(Canvas canvas) {
        ...
        // 选中当前 圆点 半径二倍
        float tmpRadius = mStepRadius * 2;
        float pivotX = getPaddingLeft() + mStepRadius
                + barWidth * (mProgress);
        RectF oval = new RectF(pivotX - tmpRadius, getPaddingTop() - mStepRadius,
                pivotX + tmpRadius, getPaddingTop() + mStepRadius * 3);
//        oval.set(pivotX - tmpRadius, getPaddingTop() - mStepRadius,
//                pivotX + tmpRadius, getPaddingTop() + mStepRadius * 3);
